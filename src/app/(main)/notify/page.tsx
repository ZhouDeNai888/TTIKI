...
export default function OrderNotifications() {
  const { list, markRead } = useNotify();
  const [mounted, setMounted] = useState<boolean>(false);
  const [expandedIds, setExpandedIds] = useState<Set<string>>(new Set());
  // minimal loading state while provider warms up
  const [loading, setLoading] = useState<boolean>(false);
  const router = useRouter();

  // if user not authenticated, redirect similar to previous behavior
  useEffect(() => {
  ...
  const unreadCount = list.filter((ev) => {
    try {
      const p = ev.payload ?? {};
      const s = String(p._meta_status ?? "").toLowerCase();
      return s === "unread";
    } catch (e) {
      return false;
    }
          ...
          <div className="flex items-center gap-3">
            ...
            {mounted && unreadCount > 0 && (
              <div className="flex items-center">
                <div className="inline-flex items-center justify-center min-w-[28px] h-7 px-2 rounded-full bg-red-600 text-white text-xs font-semibold shadow-sm">
        ...
        <ul className="space-y-3">
          {loading
            ...
            : pagedList.map((ev) => {
                ...
                // show a small type badge for info/warning/error
                const typeBadge = (() => {
                  const t = (ev.type || "").toLowerCase();
                  if (t === "info")
                    return (
                  ...
                  <li key={ev.id} className="relative">
                    ...
                    <div className="ml-3 bg-white dark:bg-slate-900 border-gray-200 dark:border-slate-700 rounded-md shadow-sm hover:shadow-md transition-shadow overflow-hidden relative">
                      <button
                        type="button"
                        onClick={async () => {
                          const wasExpanded = expandedIds.has(ev.id);
                          // toggle expansion
                          setExpandedIds((s) => {
                          ...
                          // if unread, mark as read (server + local)
                          try {
                            const p = ev.payload ?? {};
                            const status = String(
                              p._meta_status ?? ""
                            ...
                            const notificationId =
                              p._meta_notification_id ?? undefined;
                            if (status === "unread") {
                              await markRead(notificationId as any);
                            }
                          } catch (err) {
                      ...
                      >
                        ...
                        {/* order button moved below to avoid nested buttons */}
                        {String(p._meta_status || "").toLowerCase() ===
                          "unread" && (
                          <div
                            className="flex items-center ml-3 mr-2"
                            title="New"
                          >
                            <div className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-red-600 text-white text-xs font-semibold shadow-sm">
                              N
                            </div>
                          </div>
                        )}
                      ...
                      {/* absolutely positioned action button (bottom-right) when expanded */}
                        ...
                        <div className="absolute right-3 bottom-3">
                          <button
                            ...
                            onClick={(e) => {
                              ...
                              router.push(`/orders/${String(p.order_id)}`);
                            }}
